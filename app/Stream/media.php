...
WHERE
    ...
    function subFileTextsLanguage_INSERT($language, $media) {
        $query = " INSERT INTO st_text_files (language_id, video_id) VALUES(?, ?) ON DUPLICATE KEY UPDATE language_id = ?";
        $statement = $this->conn->prepare($query);
    ...
    function audioTextLanguage_INSERT($language, $media) {
        $query = " INSERT INTO st_audio_language (languageID, videoID) VALUES(?, ?) ON DUPLICATE KEY UPDATE languageID = ?";
        $statement = $this->conn->prepare($query);
        $statement->bindParam(1, $language);
        $statement->bindParam(2, $media);
        $statement->bindParam(3, $language);
    ...
    function subFileTextsLanguage_UPDATE($media, $lang, $field, $value) {
        $query = "UPDATE st_text_files SET $field = ?  WHERE video_id = ? AND language_id =? ";
        $statement = $this->conn->prepare($query);
...
//        return$query;
    ...
    function generateContentCount($id) {
        $query = "SELECT COUNT(ID) from st_module WHERE `Subject`=:id ";
        $statement = $this->conn->prepare($query);
        $statement->bindParam(':id', $id);
    ...
    function course_DELETE($id) {
        ...
        $query = "DELETE FROM projects_dir WHERE id =:id ";
    ...
    function course_UPDATE($id, $name) {
        ...
        $query = "UPDATE projects_dir SET project_name =? WHERE id=? ";
    ...
    function content_ADD($id) {
        ...
        $query = "INSERT INTO st_module (Sort, Subject, date_added, date_updated) 
        VALUES(1, '$id', '$date', '$date')
    ...
    function content_ADD_FROM_APP($id) {
        $date = $this->defaultDate();
        $query = "INSERT INTO st_module (Sort, Subject, date_added, date_updated, ModuleName, Description) 
    ...
    function content_DELETE($id) {
        $query = "DELETE FROM st_module WHERE ID=?";
        $statement = $this->conn->prepare($query);
        $statement->bindParam(1, $id);
        return $this->returnStatement($statement);
    }
    ...
    function content_ONE($id) {
        $query = "SELECT * FROM st_module WHERE st_module.ID =:id";
        $statement = $this->conn->prepare($query);
        $statement->bindParam(':id', $id);
        return $this->returnStatement($statement);
    }
    ...
    function content_LAST_INSERT() {
        $query = "SELECT * FROM st_module WHERE st_module.ID =LAST_INSERT_ID()";
        $statement = $this->conn->prepare($query);
        return $this->returnStatement($statement);
    }

    function content_VIDTREE($id) {
        $query = "SELECT * FROM st_module WHERE st_module.`Subject` =:id";
        $statement = $this->conn->prepare($query);
        $statement->bindParam(':id', $id);
        return $this->returnStatement($statement);
    }
    ...
    function content_UPDATE($id, $module, $description) {
        $date = $this->defaultDate();
        $query = "UPDATE st_module SET ModuleName =?, Description=?, date_updated=? WHERE ID=?";
        $statement = $this->conn->prepare($query);
    ...
    function media_INSERT($content, $filename, $date, $file_size, $file_type, $sort, $client) {
        $query = "
            ...
            VALUE
                    ('$filename','$content','$file_type','$file_size','$date','$sort', '$client')";
        $statement = $this->conn->prepare($query);
        if ($statement->execute()) {
    ...
    function media_LAST_SORT($module) {
        $query = " SELECT MAX(Sort) Sort FROM st_media M WHERE M.Module =?";
        $statement = $this->conn->prepare($query);
        $statement->bindParam(1, $module);
        return $this->returnStatement($statement);
    }
    ...
    function media_files_DELETE($id) {
        $query = "DELETE FROM st_media WHERE ID=?";
        $statement = $this->conn->prepare($query);
        $statement->bindParam(1, $id);
        return $this->returnStatement($statement);
    }
    ...
    function media_files_FILENAME($id) {
        $query = "SELECT Alias FROM st_media WHERE ID=? ";
        $statement = $this->conn->prepare($query);
        $statement->bindParam(1, $id);
        if ($statement->execute()) {
            return $statement->fetchColumn();
    ...
    function media_files_ALIES($id) {
        ...
        $query = "SELECT Alias,FileName FROM st_media WHERE ID= ?";
        $statement = $this->conn->prepare($query);
        $statement->bindParam(1, $id);
        return $this->returnStatement($statement);
    }
    ...
    function media_files_ALIES_LAST_INSERT() {
        ...
        $query = "SELECT Alias FROM st_media WHERE ID= LAST_INSERT_ID()";
        $statement = $this->conn->prepare($query);
        if ($statement->execute()) {
            return $statement->fetchColumn();
        } else {
    ...
    function FILE_ID_BY_ALIES($alias) {
        ...
        $query = "SELECT ID FROM st_media WHERE Alias=?";
        $statement = $this->conn->prepare($query);
    ...
    function media_files_UPDATE($id, $desc, $name) {
        $query = "UPDATE st_media SET FileName=?, comments =? WHERE ID=?";
        $statement = $this->conn->prepare($query);
        $statement->bindParam(1, $name);
    ...
    function content_UPDATE_COMMENT($id, $text) {
        $query = "UPDATE st_module SET `comment` =? WHERE ID=?";
        $statement = $this->conn->prepare($query);
    ...
    function time_caption($id) {
        $query = "SELECT * FROM st_captionTime WHERE fileID=?";
        $statement = $this->conn->prepare($query);
        $statement->bindParam(1, $id);
        return $this->returnStatement($statement);
    }
    ...
    function time_caption_INSERT($id, $count) {
        if ($count > 0) {
            $query = "INSERT INTO st_captionTime(`begin`, fileID)SELECT
                    E.`end`,
                '$id'
                FROM	st_captiontime E WHERE	E.fileID =? ORDER BY E.time_ID DESC LIMIT 1";
        } else {
            $query = "INSERT INTO st_captionTime(`begin`, fileID) VALUES('00:00:00.100', ?) ";
        }
        $statement = $this->conn->prepare($query);
        $statement->bindParam(1, $id);
        return $this->returnStatement($statement);
    ...
    function translations_UPDATE($id, $field, $value) {
        $query = "UPDATE st_translations SET " . $field . " =? WHERE ID=?";
        $statement = $this->conn->prepare($query);
        $statement->bindParam(1, $value);
        $statement->bindParam(2, $id);
        return $this->returnStatement($statement);
    ...
    function translations_INSERT_DEFAULTS($id) {
        $query = "INSERT INTO st_translations(time_ID, `language`)
            SELECT
                    '$id',
                    x.languages_id
            FROM
    ...
    function time_caption_UPDATE($id, $field, $value) {
        $query = "UPDATE st_captionTime SET " . $field . " =? WHERE time_ID=?";
        $statement = $this->conn->prepare($query);
        $statement->bindParam(1, $value);
        $statement->bindParam(2, $id);
        return $this->returnStatement($statement);
    ...
    function subtitle_procedures_UPDATE($text) {
        $query = "INSERT INTO st_procedures(ID, `Procedure`)
                VALUES
    ...
    function subtitle_procedures() {
        $query = "SELECT * FROM st_procedures ";
        $statement = $this->conn->prepare($query);
        return $this->returnStatement($statement);
    }
    ...
    function subtitle_properties_INSERT($media, $font, $font_size, $color, $background, $text_position) {
        $query = "INSERT INTO st_subtitle_properties (font, font_size,background, text_position, color, mediaID)
                        VALUES (?,?,?,?,?,?) 
                ON DUPLICATE KEY
    ...
    function subtitle_properties($media) {
        $query = "SELECT * FROM st_subtitle_properties WHERE mediaID = ?";
        $statement = $this->conn->prepare($query);
    ...
    function audioFileDetail_INSERT($id, $begin) {
        $query = "INSERT INTO st_audio_file(
                        text_languageID,
                        BeginTime,
                        Status,
                        SortID
    ...
    function audioFileDetail_UPDATE_TEXT($itemId, $fileName, $content) {
        $query = "UPDATE st_audio_file SET Content = '$content', FileAlias= '$fileName' WHERE ID = $itemId";
        $statement = $this->conn->prepare($query);
    ...
    function audioFileDetail_UPDATE($itemId, $field, $value) {
        ...
        $query = "UPDATE st_audio_file SET $field = ?, Updated = ? WHERE ID = ?";
        $statement = $this->conn->prepare($query);
        $statement->bindParam(1, $value);
    ...
    function audioFileDetail_UPDATE_FIELDS($itemId, $field, $value) {
        $query = "UPDATE st_audio_file SET $field = ? WHERE ID = ?";
        $statement = $this->conn->prepare($query);
        $statement->bindParam(1, $value);
    ...
    function audioFileDetail_DELETE($itemId) {
        $query = "DELETE FROM st_audio_file WHERE ID = ?";
        $statement = $this->conn->prepare($query);
    ...
    function audioFile_CONTENT($id) {
        $query = "SELECT   st_audio_file.Content   FROM    st_audio_file WHERE st_audio_file.ID = ?";
        $statement = $this->conn->prepare($query);
        $statement->bindParam(1, $id);
        if ($statement->execute()) {
            return $statement->fetchColumn();
    ...
    function audioMovie_INSERT($lang, $name) {
        ...
        $query = "INSERT INTO st_audiomovie (LanguageID, ItemName, LastUpdate) VALUES(?, ?, ?)
    ...
    function audioMovie($itemId) {
        ...
        $query = "SELECT ID, ItemName, LastUpdate FROM st_audiomovie WHERE LanguageID = ? ";
    ...
    function audio_IMPORT_TIMELINE($columns, $items) {
        ...
        $insert = "INSERT INTO st_audio_file (" . implode(",", $columns) . ") VALUES " . implode(",", $items);
    ...
    function st_audio_languageGET_LAST_INSERT($videoId, $languageId) {
        ...
        if ($insert) {
            $query = " SELECT ID FROM st_audio_language WHERE ID = LAST_INSERT_ID(); ";
            $statement = $this->conn->prepare($query);
            if ($statement->execute()) {
                return $statement->fetchColumn();
            } else {
    ...
    function st_audio_language_LAST_INSERT($videoId, $languageId) {
        $query = " INSERT INTO st_audio_language (videoID, languageID)     VALUES(?, ?); ";
        $statement = $this->conn->prepare($query);
    ...
    function userStat_INSERT($user) {
        $query = " INSERT INTO st_user_preview_stat (UserId,viewed_time,host_device,media_id,ip_address,playback_position, total_playback) VALUES(?,?,?,?,?,?,?)
    ...
    function audioTextLanguage_DELETE($id) {
        $query = " DELETE FROM st_audio_language WHERE ID = ?; ";
        $statement = $this->conn->prepare($query);
        $statement->bindParam(1, $id);
        return $this->returnStatement($statement);
    }
    ...
    function filmScript_SAVE($id, $content) {
        $date = $this->defaultDate();
        $query = "INSERT INTO st_scripts (media_id, content, created_at) VALUES(?,?,?) ON DUPLICATE KEY UPDATE    
        content =?, updated_at =?";
        $statement = $this->conn->prepare($query);
        $statement->bindParam(1, $id);
    ...
    function filmScript_GET($id){
        $query = "SELECT * FROM st_scripts WHERE media_id =?";
        $statement = $this->conn->prepare($query);
        $statement->bindParam(1, $id);
        return $this->returnStatement($statement);
    }
    ...
    function mediaResources_GET($id){
        $query =
            "SELECT C.ID content_id,M.fileType media_type,M.ID media_id, C.`Subject` project_id FROM st_module C 
                    JOIN st_media M ON M.Module = C.ID WHERE M.ID = ?";
        $statement = $this->conn->prepare($query);
        $statement->bindParam(1, $id);
        return $this->returnStatement($statement);
    }
